...
namespace Omni.Core;
public sealed record MusicCandidate(string RecordingId,string? ReleaseId,string Title,string Artist,string Album,string Edition="")
{
...
public sealed partial class MusicMetadata
{
    ...
    public async Task<MusicLookup> Recording(string recordingId,string? releaseId,CancellationToken ct){
        ...
        var r=await ReadEntity($"recording/{id}?inc=artists+releases+release-groups&fmt=json",ct);
        ...
        if(selected?.ReleaseId is string rel&&Guid.TryParse(rel,out var rid)){
            var release=await ReadEntity($"release/{rid}?inc=recordings+artist-credits&fmt=json",ct);
        ...
        return new(cover is null?MusicLookupState.MatchedNoCover:MusicLookupState.Matched,tags["TIT2"],tags["TPE1"],album,cover,Tags:tags);
    }
    ...
    public async Task<MusicLookup> Scan(string file,string ffmpeg,string key,double? duration,CancellationToken ct){
    ...
    public async Task<MusicLookup> Recognize(string title,string artist,double? duration,string file,string ffmpeg,string key,CancellationToken ct){
        var text=await Lookup(title,artist,duration,ct);if(text.State is MusicLookupState.Matched or MusicLookupState.MatchedNoCover)return text;
