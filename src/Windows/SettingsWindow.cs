...
public sealed class SettingsWindow : Window
{
    public SettingsWindow(Window owner, Downloader engine)
    {
        ...
        var download = new Button { Content = "下載並安裝最新版本" }; download.Click += async (_, _) => { try { Save(); if (!await new ConfirmWindow(this,"安裝更新","更新會自動暫停未完成下載、關閉 App 並重新啟動。設定及歷史紀錄會保留。","下載並安裝").Ask()) return; download.IsEnabled = false; status.Text = "正在下載並驗證更新包…"; await UpdateManager.DownloadAndLaunch(p.ReleaseRepository, elevated.IsChecked == true, true, new Progress<string>(text => status.Text = text)); status.Text = "已驗證更新包，準備重新啟動。"; } catch (Exception e) { status.Text = e.Message; } finally { download.IsEnabled = true; } }; panel.Children.Add(download);
    }
}
