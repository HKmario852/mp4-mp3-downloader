...
public class TagEditingTests
{
    [Fact] public async Task CommentsCoversAndOptionalRenamePreserveAudio()
    {
        ...
        try{
            ...
            Assert.Equal(source,j.FilePath);var d=Id3Document.Read(source);Assert.Equal("廣東話註解",d.Text("COMM"));Assert.Equal("專輯演出者",d.Text("TPE2"));Assert.Equal(art,d.GetCovers().Single().Bytes);Assert.Equal(audio,File.ReadAllBytes(source).Skip((int)d.AudioOffset).ToArray());
