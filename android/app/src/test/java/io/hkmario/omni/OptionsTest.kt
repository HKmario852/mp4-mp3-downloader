...
class OptionsTest {
 ...
 @Test fun mp3DoesNotRequestStandaloneJpeg(){val prefs=Prefs(keepThumbnail=true,embedThumbnail=true);assertFalse(Options.format(TaskItem(url="https://example.org",mode="mp3",outputFormat="mp3"),prefs).any{it.first=="--write-thumbnail"});assertTrue(Options.format(TaskItem(url="https://example.org",mode="mp4",outputFormat="mp4"),prefs).any{it.first=="--write-thumbnail"})}
}
