...
class Engine(val context: Context) {
    ...
    suspend fun cancel(ids: Set<String>) {for(id in ids){val t=get(id);if(t.state in listOf(State.Completed,State.Cancelled))continue;update(id){it.copy(state=State.Cancelled,speed=0.0,eta=0)};YoutubeDL.getInstance().destroyProcessById(id);active[id]?.cancelAndJoin();val root=File(context.filesDir,"work").canonicalFile;val dir=File(root,id).canonicalFile;require(dir.parentFile==root);dir.deleteRecursively();grants.tasks.remove(id)}}
