...
class TagEditor(private val engine:Engine) {
    ...
    suspend fun apply(ids:Set<String>,delta:Map<String,String>,raw:Map<String,ByteArray>,cover:ByteArray?)=lock.withLock { withContext(Dispatchers.IO) {
        ...
        ids.forEach { id ->
            ...
            val tag=Id3.read(source);delta.forEach{(k,v)->tag.setText(k,v)};raw.forEach{(k,v)->tag.setRaw(k,v)}
            if(cover!=null)tag.covers(listOf(Art(cover,if(cover[0]==0xff.toByte())"image/jpeg"else"image/png","User cover",3)))
            ...
            try {
                ...
                if(content) {
                    ...
                    if(delta.containsKey("TIT2")&&!delta["TIT2"].isNullOrEmpty()){
                    ...
                    if(delta.containsKey("TIT2")&&!delta["TIT2"].isNullOrEmpty()&&delta["TIT2"]!=source.nameWithoutExtension){var target=File(source.parentFile,delta["TIT2"]+".mp3");var i=1;while(target.exists())target=File(source.parentFile,"${delta["TIT2"]} (${i++}).mp3");destination=target.absolutePath}
                    source.copyTo(backup)
