...
@Composable fun LibraryScreen(engine:Engine,open:(TaskItem)->Unit,onEdit:(Set<String>)->Unit = {}){val tasks by engine.tasks.collectAsStateWithLifecycle();val p by engine.prefs.collectAsStateWithLifecycle();val scope=rememberCoroutineScope();var q by remember{mutableStateOf("")};var format by remember{mutableStateOf("all")};var date by remember{mutableIntStateOf(0)};var order by remember{mutableIntStateOf(0)};var selected by remember{mutableStateOf(setOf<String>())};var detail by remember{mutableStateOf<TaskItem?>(null)};var deleting by remember{mutableStateOf<List<TaskItem>?>(null)};var rename by remember{mutableStateOf<TaskItem?>(null)};var editing by remember{mutableStateOf<Set<String>?>(null)};var clearing by remember{mutableStateOf<Set<String>?>(null)};var message by remember{mutableStateOf<String?>(null)};var trashPending by remember{mutableStateOf<List<TaskItem>>(emptyList())};var permanentPending by remember{mutableStateOf<List<TaskItem>>(emptyList())}
 ...
 Column(Modifier.fillMaxSize()){
  ...
  BoxWithConstraints(Modifier.fillMaxWidth()){
   val stats=listOf(text(p,"已完成","Completed") to library.size.toString(),text(p,"影片","Video") to library.count{it.mode=="mp4"}.toString(),text(p,"音訊","Audio") to library.count{it.mode=="mp3"}.toString(),text(p,"總大小","Total size") to "%.2f GB".format(library.sumOf{it.total?:0}/1e9))
   val columns=if(maxWidth<600.dp)2 else 4
   Column(verticalArrangement=Arrangement.spacedBy(6.dp)){stats.withIndex().toList().chunked(columns).forEach{group->Row(Modifier.fillMaxWidth().height(IntrinsicSize.Min),horizontalArrangement=Arrangement.spacedBy(6.dp)){group.forEach{(index,pair)->Card(Modifier.weight(1f).fillMaxHeight()){Row(Modifier.padding(12.dp).heightIn(min=54.dp),verticalAlignment=Alignment.CenterVertically){FeatureIcon(listOf("complete","video","audio","size")[index],Modifier.size(26.dp));Spacer(Modifier.width(10.dp));Column(Modifier.weight(1f)){Text(pair.first,fontSize=11.sp);Text(pair.second,fontSize=18.sp)}}}}}}}
  }

  Row(Modifier.horizontalScroll(rememberScrollState()),verticalAlignment=Alignment.CenterVertically){AppCheckbox(shown.isNotEmpty()&&shown.all{it.id in selected},{checked->selected=if(checked)selected+shown.map{it.id}else selected-shown.map{it.id}.toSet()});Text(text(p,"全選","Select all"));TextButton(enabled=selected.isNotEmpty(),onClick={deleting=library.filter{it.id in selected}}){FeatureIcon("delete");Spacer(Modifier.width(6.dp));Text(text(p,"移除所選","Remove selected"))};TextButton(enabled=shown.isNotEmpty(),onClick={clearing=shown.map{it.id}.toSet()}){Text(text(p,"清除紀錄","Clear history"))};TextButton(enabled=selected.any{id->library.any{it.id==id&&it.extension=="mp3"}},onClick={editing=library.filter{it.id in selected&&it.extension=="mp3"}.map{it.id}.toSet()}){Text(text(p,"MP3 標籤編輯","Edit MP3 tags"))}}
  LazyColumn(Modifier.weight(1f),verticalArrangement=Arrangement.spacedBy(8.dp)) {
   items(shown,key={it.id}) { t ->
    var menu by remember(t.id){mutableStateOf(false)}
    Card(colors=CardDefaults.cardColors(containerColor=if(t.id in selected)MaterialTheme.colorScheme.primaryContainer else MaterialTheme.colorScheme.surface),modifier=Modifier.fillMaxWidth().combinedClickable(onClick={detail=t;selected=if(selected.size>=2){if(t.id in selected)selected-t.id else selected+t.id}else setOf(t.id)},onLongClick={selected=selected+t.id})) {
     Row(Modifier.padding(10.dp),verticalAlignment=Alignment.CenterVertically) {
      if(selected.size>=2)AppCheckbox(t.id in selected,{v->selected=if(v)selected+t.id else selected-t.id})
      AsyncImage(t.thumbnail,null,Modifier.size(64.dp,48.dp))
      Column(Modifier.weight(1f).padding(horizontal=12.dp)) {
       Text(t.title,maxLines=2,fontWeight=FontWeight.SemiBold)
       Row(verticalAlignment=Alignment.CenterVertically){FeatureIcon(if(t.extension in listOf("mp3","mp4"))t.extension else "video",Modifier.size(18.dp));Spacer(Modifier.width(5.dp));Text("${t.extension.uppercase()} · ${quality(t)} · ${size(t)}",fontSize=12.sp)}
 ...
 editing?.let{ids->LaunchedEffect(ids){editing=null;onEdit(ids)}}
