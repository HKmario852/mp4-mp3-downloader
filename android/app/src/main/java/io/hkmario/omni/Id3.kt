...
class Id3 private constructor(val version: Int,val audioOffset: Long,val frames: MutableList<Frame>) {
    ...
    fun text(id: String): String { val b=frames.firstOrNull{it.id==id}?.data ?: return "";if(b.size<2)return "";val charset=when(b[0].toInt()){0->Charsets.ISO_8859_1;1->Charsets.UTF_16;2->Charsets.UTF_16BE;3->Charsets.UTF_8;else->return ""};return String(b,1,b.size-1,charset).trimEnd('\u0000') }
    ...
    fun setText(id: String,value: String) { require(id.matches(Regex("T[A-Z0-9]{3}")) && id!="TXXX");setRaw(id,byteArrayOf(1,0xff.toByte(),0xfe.toByte())+value.toByteArray(Charsets.UTF_16LE)+byteArrayOf(0,0)) }
