...
class Id3 private constructor(val version: Int,val audioOffset: Long,val frames: MutableList<Frame>) {
    ...
    fun text(id: String): String { if(id.startsWith("TXXX:"))return frames.filter{it.id=="TXXX"}.map{decodeCustom(it.data).split('\u0000',limit=2)}.firstOrNull{it.size==2&&it[0]==id.substring(5)}?.get(1).orEmpty();if(id=="COMM")return comment(); val b=frames.firstOrNull{it.id==id}?.data ?: return "";if(b.size<2)return "";val charset=when(b[0].toInt()){0->Charsets.ISO_8859_1;1->Charsets.UTF_16;2->Charsets.UTF_16BE;3->Charsets.UTF_8;else->return ""};return String(b,1,b.size-1,charset).trimEnd('\u0000') }
    ...
    private fun decodeCustom(b:ByteArray):String=if(b.size<2)""else String(b,1,b.size-1,when(b[0].toInt()){0->Charsets.ISO_8859_1;1->Charsets.UTF_16;2->Charsets.UTF_16BE;else->Charsets.UTF_8}).trimEnd('\u0000')
    fun setText(id: String,value: String) { if(id.startsWith("TXXX:")){val name=id.substring(5);val payload=byteArrayOf(1,0xff.toByte(),0xfe.toByte())+(name+"\u0000"+value+"\u0000").toByteArray(Charsets.UTF_16LE);val index=frames.indexOfFirst{it.id=="TXXX"&&decodeCustom(it.data).substringBefore('\u0000')==name};val frame=Frame("TXXX",byteArrayOf(0,0),payload);if(index<0)frames+=frame else frames[index]=frame;return}; if(id=="COMM"){setRaw(id,byteArrayOf(1,101,110,103,0xff.toByte(),0xfe.toByte(),0,0,0xff.toByte(),0xfe.toByte())+value.toByteArray(Charsets.UTF_16LE)+byteArrayOf(0,0));return}; require(id.matches(Regex("T[A-Z0-9]{3}")) && id!="TXXX");setRaw(id,byteArrayOf(1,0xff.toByte(),0xfe.toByte())+value.toByteArray(Charsets.UTF_16LE)+byteArrayOf(0,0)) }
